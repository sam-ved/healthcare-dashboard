...
import { createPatient, deletePatient, listPatients, togglePatientStatus, } from "../services/patientService.js";
import { sendRegistrationSMS } from "../services/smsService.js";
export async function createPatientHandler(req, res) {
    const { name, dob, issue, since, fullName, gender, phone, address, bloodGroup, allergies, weight, height } = req.body;
...
export async function togglePatientStatusHandler(req, res) {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
        return res.status(400).json({ message: "Invalid patient id" });
    }
